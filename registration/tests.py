    ...
    def test_registration_form(self):
        ...
        invalid_data_dicts = [
            ...
            { 'username': 'foo/bar',
              ...
              'password2': 'foo' },
            'error':
            ('username', [u'Username can only contain alphanumeric characters and underscores' ])
            },
...
class RegistrationViewTests(RegistrationTestCase):
    ...
    """
    def test_1_registration_view(self):
        """
    ...
    def test_1_registration_view_no_errors(self):
        response = self.client.post(reverse('registration_register'),
                                    data={ 'username': 'foo',
                                           'email': 'foo@example.com',
                                           'password1': 'foo',
...
#        self.assertEqual(RegistrationProfile.objects.count(), 2)
    ...
    def test_2_activation_view(self):
        """
        ...
        self.assertEqual(response.status_code, 200)
        #self.assertEqual(response.context[4]['account'].pk if len(response.context) >= 5 and 'account' in response.context[4] else response.context[0]['account'], self.sample_user.pk)

    def test_3_activation_view(self):
